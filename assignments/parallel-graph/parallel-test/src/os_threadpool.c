...
void enqueue_task(os_threadpool_t *tp, os_task_t *t)
{
	...
	pthread_mutex_lock(&tp->no_task_mutex);

	list_add_tail(&tp->head, &t->list);
	pthread_cond_signal(&tp->no_task_cond);

	pthread_mutex_unlock(&tp->no_task_mutex);
}
...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	while (queue_is_empty(tp) && tp->remaining)
		pthread_cond_wait(&tp->no_task_cond, &tp->no_task_mutex);
...
static void *thread_loop_function(void *arg)
{
	...
	while (1) {
		...
		if (t == NULL)
			break;
		((thread_arg *)(t->argument))->count = &nodes;
		t->action(t->argument);
		destroy_task(t);
	}

	printf("num processed nodes: %u\n", nodes);
...
void wait_for_completion(os_threadpool_t *tp)
{
	...
	while (tp->remaining)
		pthread_cond_wait(&tp->done_cond, &tp->done_mutex);

	pthread_cond_broadcast(&tp->no_task_cond);
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	pthread_mutex_init(&tp->no_task_mutex, NULL);
	pthread_mutex_init(&tp->done_mutex, NULL);
	pthread_cond_init(&tp->done_cond, NULL);
	pthread_cond_init(&tp->no_task_cond, NULL);
...
void destroy_threadpool(os_threadpool_t *tp)
{
	...
	pthread_mutex_destroy(&tp->no_task_mutex);
	pthread_mutex_destroy(&tp->done_mutex);
	pthread_cond_destroy(&tp->no_task_cond);
